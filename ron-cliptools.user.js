...
// @version      6.0.0
// @description  RON ClipTools for BuildNow.GG - rolling gameplay clips and screenshots
// @author       Ron
...
(function () {
    ...
    const canvas = () => {
        const list = [...document.querySelectorAll('canvas')].filter(c => c.width && c.height);
    ...
    function updateUI() {
        ...
        if (t) t.textContent = state.recording ? 'Recording recent gameplay' : 'Buffer is off';
    }
    ...
    function startBuffer() {
        ...
        try {
            // IMPORTANT: This uses the BuildNow canvas directly.
            // There is NO getDisplayMedia(), so there is NO screen/window picker,
            // no browser capture permission prompt, and no "cancelled or blocked" error.
            ...
            updateUI();
            status(`Buffer ON — collecting last ${settings.duration}s`);
        } catch (e) {
            ...
            updateUI();
            status('Could not start game recording');
        }
    }
    ...
    function saveLastClip() {
        ...
        // Keep one earlier chunk for WebM initialization/keyframe continuity.
        ...
        const blob = new Blob(selected, {type: state.recorder.mimeType || 'video/webm'});
    ...
    function ui() {
        ...
        <button id="shot">Screenshot</button><button id="buffer">Start Clip Buffer</button><button id="clip" disabled>Save Last ${settings.duration}s Clip</button><div id="record-timer">Buffer is off</div>
        <button id="website">RON Labs Clips</button><div class="small">The buffer records the BuildNow game canvas only. Start it once, wait for footage, then press your clip key to save the last selected seconds. No screen-share picker is used.</div>
        <div id="settings"><div class="row"><span>Clip length</span><select id="duration"><option value="5">5 seconds</option><option value="10">10 seconds</option><option value="15">15 seconds</option><option value="30">30 seconds</option><option value="60">60 seconds</option><option value="120">120 seconds</option></select></div>
        <div class="row"><span>Screenshot key</span><input id="shot-key" value="${escape(settings.screenshot)}"></div><div class="row"><span>Clip key</span><input id="clip-key" value="${escape(settings.clip)}"></div><div class="row"><span>Window key</span><input id="panel-key" value="${escape(settings.panel)}"></div></div><div id="status">Ready</div></div>`;
        shadow.appendChild(p);

        const d=shadow.querySelector('#duration'); d.value=String(settings.duration);
        d.addEventListener('change',()=>{settings.duration=Number(d.value)||15;saveSettings();const b=shadow.querySelector('#clip');if(b)b.textContent=`Save Last ${settings.duration}s Clip`;status(`Clip length: ${settings.duration}s`);});
        ...
        const sk=shadow.querySelector('#shot-key'), ck=shadow.querySelector('#clip-key'), pk=shadow.querySelector('#panel-key');
        ...
        drag(p,shadow.querySelector('#head')); updateUI();
    }
    ...
    document.addEventListener('keydown',e=>{
        ...
        if(keyMatches(e,settings.screenshot)){e.preventDefault();screenshot();}
        else if(keyMatches(e,settings.clip)){e.preventDefault();saveLastClip();}
        else if(keyMatches(e,settings.panel)){e.preventDefault();togglePanel();}
    },false);

    function mount(){
        if(!document.body||document.getElementById(ID))return;
        document.body.appendChild(root); ui();
    }
