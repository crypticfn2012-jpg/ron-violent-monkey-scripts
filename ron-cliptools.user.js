...
(function () {
    ...
    const state = { recording:false, recorder:null, stream:null, chunks:[], timer:null, folderHandle:null };
    const settings = {
    ...
    async function startRecording() {
        ...
        try {
            ...
            console.log('[RON ClipTools] Capture:',err);
    ...
    function createUI(){
        ...
        const sk=shadow.querySelector('#shot-key'),rk=shadow.querySelector('#record-key'),pk=shadow.querySelector('#panel-key');
        sk.addEventListener('change',()=>{settings.screenshot=sk.value.trim()||'F8';saveSettings();});
        rk.addEventListener('change',()=>{settings.record=rk.value.trim()||'F9';saveSettings();});
    ...
    document.addEventListener('keydown',e=>{
        ...
        else if(keyMatches(e,settings.record)){e.preventDefault();state.recording?stopRecording():startRecording();}
