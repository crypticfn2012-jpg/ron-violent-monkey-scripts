...
(function () {
    ...
    // Deliberately use no shared globals, no page hooks, no Shadow DOM and no
    // dependencies. This version is designed to survive alongside the other
    // Ron BuildNow userscripts.
    var ID = '__RON_CLIPTOOLS_V900__';
    var panel;
    var settings = {
        shot: 'F8',
        clip: 'F9',
    ...
    function setStatus(text) {
        var e = panel && panel.querySelector('#rc9-status');
    ...
    function updateButtons() {
        if (!panel) return;
        var b = panel.querySelector('#rc9-buffer');
        var s = panel.querySelector('#rc9-save');
        var t = panel.querySelector('#rc9-timer');
    ...
    function build() {
        ...
        panel.innerHTML = '<div id="rc9-head" style="padding:14px 15px;background:#101712;color:#fff;cursor:move;user-select:none"><b style="font-size:17px;color:#35ff83">RON ClipTools</b><div style="font-size:11px;color:#718078;margin-top:3px">RON Labs • BuildNow.GG</div></div>' +
            '<div style="padding:12px">' +
            '<button id="rc9-shot">Screenshot</button>' +
            '<button id="rc9-buffer">Start Clip Buffer</button>' +
            '<button id="rc9-save" disabled>Save Last ' + settings.duration + 's</button>' +
            '<div id="rc9-timer" style="text-align:center;color:#35ff83;font-size:11px;margin:2px 0 9px">Buffer is off</div>' +
            '<button id="rc9-site">RON Labs Clips</button>' +
            ...
            '<label style="display:flex;justify-content:space-between;align-items:center;color:#aab5ae;font-size:12px;margin:8px 0">Clip length <select id="rc9-duration"><option value="5">5 seconds</option><option value="10">10 seconds</option><option value="15">15 seconds</option><option value="30">30 seconds</option><option value="60">60 seconds</option><option value="120">120 seconds</option></select></label>' +
            '<label style="display:flex;justify-content:space-between;align-items:center;color:#aab5ae;font-size:12px;margin:8px 0">Screenshot key <input id="rc9-sk"></label>' +
            '<label style="display:flex;justify-content:space-between;align-items:center;color:#aab5ae;font-size:12px;margin:8px 0">Clip key <input id="rc9-ck"></label>' +
            '<label style="display:flex;justify-content:space-between;align-items:center;color:#aab5ae;font-size:12px;margin:8px 0">Window key <input id="rc9-pk"></label>' +
            '<div id="rc9-status" style="text-align:center;color:#7a867e;font-size:11px;line-height:14px;min-height:28px;padding-top:2px">RON ClipTools loaded</div>' +
            '</div>';

        var style = document.createElement('style');
        style.textContent = '#' + ID + ' button{width:100%;box-sizing:border-box;border:0;border-radius:9px;padding:10px;margin:0 0 8px;background:#1a241d;color:#fff;font-weight:700;cursor:pointer}#' + ID + ' button:hover{background:#26362b}#' + ID + ' button:disabled{opacity:.4;cursor:not-allowed}#' + ID + ' #rc9-shot,#' + ID + ' #rc9-save{background:#168c46}#' + ID + ' #rc9-buffer{background:#214d32}#' + ID + ' #rc9-site{background:#35ff83;color:#061008}#' + ID + ' input,#' + ID + ' select{width:108px;box-sizing:border-box;background:#080b09;color:#fff;border:1px solid #303b33;border-radius:6px;padding:6px}';
        document.documentElement.appendChild(style);
        document.body.appendChild(panel);

        panel.querySelector('#rc9-sk').value = settings.shot;
        panel.querySelector('#rc9-ck').value = settings.clip;
        panel.querySelector('#rc9-pk').value = settings.panel;
        panel.querySelector('#rc9-duration').value = String(settings.duration);

        panel.querySelector('#rc9-shot').onclick = screenshot;
        panel.querySelector('#rc9-buffer').onclick = function(){ recording ? stopBuffer() : startBuffer(); };
        panel.querySelector('#rc9-save').onclick = saveClip;
        panel.querySelector('#rc9-site').onclick = function(){ window.open('https://ron.cool/clips', '_blank'); };
        panel.querySelector('#rc9-duration').onchange = function(e){ settings.duration=Number(e.target.value)||15; saveSettings(); updateButtons(); };
        panel.querySelector('#rc9-sk').onchange = function(e){ settings.shot=e.target.value.trim()||'F8'; saveSettings(); };
        panel.querySelector('#rc9-ck').onchange = function(e){ settings.clip=e.target.value.trim()||'F9'; saveSettings(); };
        panel.querySelector('#rc9-pk').onchange = function(e){ settings.panel=e.target.value.trim()||'F7'; saveSettings(); };

        var head = panel.querySelector('#rc9-head');
    ...
    // Wait for BuildNow's body, then keep the UI mounted if another script removes it.
    var tries = 0;
